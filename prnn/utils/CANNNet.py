...
class CANNnet(PredictiveNet):
    ...
    def predict(self, obs, act, state):
        ...
        if hasattr(self, "trainNoiseMeanStd") and self.trainNoiseMeanStd != (0, 0):
            ...
            noise_t = noise[0] + noise[1] * torch.randn(
                (1, timesteps, self.hidden_size), device=device
            )
        else:
            noise_t = torch.tensor([])

        obs_pred, h, obs_next, self.current_state = self.pRNN(
            state, None, noise_t, self.current_state
        )
    ...
    def env2pred_state(self, state):
        ...
        if isinstance(state, dict):
            state = torch.tensor(
                state["agent_pos"][:-1, :], dtype=torch.float, requires_grad=False
            )
        elif isinstance(state, tuple):
            state = torch.tensor(
                state, dtype=torch.float, requires_grad=False, device=self.pRNN.W.device
            )[None]
        else:
...
class CANNRNN(nn.Module):
    ...
    def position2input(self, position):
        ...
        cellinput = I0 + lambd * np.exp(
            -cdist(position, self.locations[0]) / tuning_width
        )
