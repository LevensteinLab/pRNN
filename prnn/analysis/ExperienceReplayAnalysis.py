...
class ExperienceReplayAnalysis:
    def __init__(
        ...
        self.lrs = np.concatenate(
            (np.array([0]), np.logspace(np.log10(lrmin), np.log10(lrmax), numlrs))
        )
        ...
        for expIDX, exp in enumerate(self.exps):
            ...
            LRpanel, pTimeWake, pVisited, SWcorr = self.runLRpanel(
                self.pN,
                self.lrs,
                exp,
                numWakeRepeats,
    ...
    def getExamples(
        self, seqdur, sleepNoiseStd=0.1, hiExampleThresh=0.5, maxSleepRepeats=10000
    ):
        self.HiExample = None
        self.LowExample = None
        self.trialExample = None
    ...
    def replayScore(self, occupancyTrial, occupancyReplay):
        ...
        pWakeVisited = np.sum((occupancyReplay > 0) * (occupancyTrial > 0)) / np.sum(
            occupancyTrial > 0
        )
    ...
    def replayTrial(
        ...
        OTA_trial = OTA(
            ...
            timesteps_sleep=timesteps_sleep,
            suppressPrint=True,
            withTransitionMaps=False,
            sleepOnsetTransient=onsetTransient,
            skipContinuity=True,
        ...
        replayScore, pTimeInWakeLoc, pWakeVisited = self.replayScore(
            trialOccupancy, occupancyMap
        )

        if (
            lr > self.exampleLRthresh
            and occ_corr > self.hiExampleThresh
            and self.HiExample is None
        ):
            self.HiExample = {
                "traj": OTA_trial.SLEEPdecoded[0],
        ...
        if (
            lr > self.exampleLRthresh
            and occ_corr < self.lowExampleThresh
            and self.LowExample is None
        ):
            self.LowExample = {
                "traj": OTA_trial.SLEEPdecoded[0],
                "coverage": OTA_trial.coverageContinuity[2],
    ...
    def runReplayTrials(
        ...
        for wakeIDX in range(numWakeRepeats):
            ...
            if resetOptimizer:
                ...
                pN_post.resetOptimizer(
                    pN_post.trainArgs.lr, pN_post.trainArgs.weight_decay
                )
    ...
    def runLRpanel(
        ...
        for lrIDX, lr in enumerate(lrs):
            ...
            (occ_overlap, replayScore, pTimeInWakeLoc, pWakeVisited) = (
                self.runReplayTrials(
                    pN,
                    exp,
                    lr,
                    numWakeRepeats,
    ...
    def collectExperience(self, pN, seqdur):
        ...
        obs, act, state, render = pN.collectObservationSequence(
            env, agent, seqdur, includeRender=True
        )
        lastRender = render[-1]
        ...
        traj = nap.TsdFrame(
            t=np.arange(timesteps), d=traj, time_units="s", columns=("x", "y")
        )

        coverage = trajectoryAnalysis.calculateCoverage(
            traj, [0, env.width, 0, env.height]
        )
    ...
    def LRPanelFigure(self, netname=None, savefolder=None):
        plt.figure()
        plt.subplot(3, 3, 1)
        self.plotTrajectoryPanel(
            self.trialExample["traj"],
            self.trialExample["lastRender"],
            showRender=True,
            showOccupancy=False,
        )
        ...
        if self.LowExample is not None:
            plt.subplot(3, 3, 3)
            self.plotTrajectoryPanel(
                self.LowExample["traj"],
                self.trialExample["coverage"],
                timeMarker=True,
                showOccupancy=True,
            )
        ...
        if netname is not None:
            saveFig(plt.gcf(), netname + "_LRReplayPanel", savefolder, filetype="png")

        # plt.show()

    def plotLRPanel(
        self, occ_overlap, lrs, meanLR=None, metric="Trial-Sleep Corr.", showxlabel=True
    ):
    ...
    def plotReplayScorePanel(self, meanpWake, meanpVisited, lrs):
        plt.plot(
            np.arange(len(lrs)) + 1,
            np.mean(meanpWake, axis=1),
            label="P[Time] in Wake Locs",
        )
        plt.plot(
            np.arange(len(lrs)) + 1,
            np.mean(meanpVisited, axis=1),
            label="P[Wake Locs] visited",
        )
        plt.legend()

    def plotTrajectoryPanel(
        self,
        traj,
        coverage,
        trajRange=None,
        ...
        if showOccupancy:
            ...
            plt.imshow(np.flip(coverage, axis=0), extent=(-0.5, 17.5, -0.5, 17.5))
        else:
            plt.imshow(
