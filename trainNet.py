...
parser.add_argument(
    "--pRNNtype", default="thRNN_2win", help="which pRNN (Default: thRNN_2win)"
)
...
parser.add_argument(
    "--loadfolder", default="", help="Where to load the net? (foldername/)"
)
...
parser.add_argument(
    "--hiddensize", default=500, type=int, help="how many hidden units? (Default: 500"
)

parser.add_argument(
    "-c", "--contin", action="store_true", help="Continue previous training?"
)
...
parser.add_argument(
    "-s", "--seed", default=8, type=int, help="Random Seed? (Default: 8)"
)
...
parser.add_argument(
    "--bptttrunc", default=1e8, type=int, help="BPTT Truncation window? (Default: 1e8)"
)
...
if predictiveNet.numTrainingTrials == -1:
    ...
    predictiveNet.trainingEpoch(
        env, agent, sequence_duration=sequence_duration, num_trials=1
    )
    predictiveNet.useDataLoader = args.withDataLoader
...
while predictiveNet.numTrainingEpochs < numepochs:
    ...
    predictiveNet.plotLearningCurve(
        savename=savename, savefolder=figfolder, incDecode=True
    )
