...
def TrainingFigure(predictiveNet, savename=None, savefolder=None, incDecode=True):
    ...
    predictiveNet.plotTuningCurvePanel(fig=finalPFfig, place_fields=place_fields, SI=SI)
    ...
    for PFpanel in range(3):
        ...
        predictiveNet.plotTuningCurvePanel(
            fig=PFfigs[PFpanel], place_fields=place_fields, SI=None
        )

    if savename is not None:
        saveFig(plt.gcf(), savename + "_TrainingFigure", savefolder, filetype="png")
    # plt.show()

    return


def SpontActivityFigure(
    predictiveNet,
    compareWAKEagent=None,
    murange=0.5,
    maxstd=1,
    numpoints=11,
    timesteps=1000,
    ...
    for i, noisemag in enumerate(noisemags):
        ...
        for j, noisestd in enumerate(noisestds):
            while meanrate[i, j] > 5 and simcounts[i, j] < 5:
                obs_pred, h, noise_t = predictiveNet.spontaneous(
                    timesteps, noisemag, noisestd
                )
                ...
                if decoder:
                    ...
                    c = trajectoryAnalysis.calculateCoverage(
                        decoded[onset:], [1, decoder.gridheight, 1, decoder.gridwidth]
                    )
                    coverage[i, j] = c["nonuniformity"]
                    c = trajectoryAnalysis.calculateContinuity(
                        decoded[onset:], showFig=False
                    )
    ...
    if compareWAKEagent:
        ...
        obs, act, state, render = predictiveNet.collectObservationSequence(
            env, agent, timesteps, includeRender=False
        )
    ...
    for s, sample in enumerate(examples):
        obs_pred, h[s], noise_t = predictiveNet.spontaneous(
            timesteps, sample[0], sample[1]
        )
    ...
    plt.imshow(
        ...
        cmap="twilight",
        vmin=np.log10(meanWAKE) - 2.5,
        vmax=np.log10(meanWAKE) + 2.5,
    )
    ...
    plt.imshow(
        ...
        extent=(-murange, murange, np.log10(noisestds[0]), np.log10(noisestds[-1])),
        cmap="twilight",
        vmin=np.log10(stdWAKE) - 1.5,
        vmax=np.log10(stdWAKE) + 1.5,
    )
    plt.plot([0, 0], np.log10(noisestds[[0, -1]]), "w--")
    plt.colorbar()
    ...
    plt.plot(
        np.log10(noisestds),
        meanWAKE * np.ones(np.shape(noisestds)),
        "r-",
        linewidth=1,
        label="WAKE",
    )
    ...
    for s, sample in enumerate(examples):
        plt.hist(
            (actStats[s]["meancellrates"]),
            histtype="step",
            bins=np.linspace(0, 1, 21),
            label=f"noise:{examples[s][1]}",
            color=np.array([1, 1, 1]) / (s + 2),
            linewidth=1,
        )
    plt.legend()
    plt.xlabel("Mean FR")
    ...
    if savename is not None:
        saveFig(
            plt.gcf(), savename + "_SpontActivityFigure", savefolder, filetype="png"
        )

    # plt.show()


def SpontActivityExamples(
    predictiveNet,
    examples=((0, 0.1), (0, 0.25), (0, 0.4)),
    timesteps=4000,
    savename=None,
    ...
    for s, sample in enumerate(examples):
        ...
        if decoder:
            ...
            coverage[s] = trajectoryAnalysis.calculateCoverage(
                decoded[onset:], [1, decoder.gridheight, 1, decoder.gridwidth]
            )
            continuity[s] = trajectoryAnalysis.calculateContinuity(
                decoded[onset:], showFig=False
            )
    ...
    for s, sample in enumerate(examples):
        ...
        plt.imshow(
            coverage[s]["occupancy"].transpose(),
            interpolation="nearest",
            alpha=decoder.mask.transpose(),
        )
    ...
    if savename is not None:
        saveFig(
            plt.gcf(), savename + "_SpontaneousExamples", savefolder, filetype="png"
        )

    # plt.show()


def SpontTrajectoryFigure(
    predictiveNet,
    decoder,
    noisemag=0,
    noisestd=0.25,
    ...
    coverage = trajectoryAnalysis.calculateCoverage(
        decoded, [1, decoder.gridheight, 1, decoder.gridwidth]
    )
    ...
    predictiveNet.plotSequence(
        predictiveNet.env_shell.pred2np(
            obs_pred, timesteps=range(timesteps - 6, timesteps)
        ),
        range(6),
        4,
        label="Predicted",
    ...
    predictiveNet.plotSequence(
        np.transpose(p, axes=[0, 2, 1]),
        range(timesteps - 7, timesteps - 1),
        5,
        label="Decoded",
        mask=decoder.mask.transpose(),
    )

    if savename is not None:
        saveFig(
            plt.gcf(), savename + "_SpontaneousTrajectory", savefolder, filetype="png"
        )

    # plt.show()


def OfflineTrajectoryProps(
    predictiveNet,
    decoder,
    timesteps=5000,
    noisemag=0,
    ...
    for j, noisestd in enumerate(noisestds):
        ...
        while meanrate > 5:
            ...
            coverage[j] = trajectoryAnalysis.calculateCoverage(
                decoded, [1, decoder.gridheight, 1, decoder.gridwidth]
            )
    ...
    for e, eidx in enumerate(examples):
        plt.subplot(4, 5, e + 1)
        plt.imshow(
            coverage[eidx]["occupancy"].transpose(),
            interpolation="nearest",
            alpha=decoder.mask.transpose(),
        )
        # plt.xticks([])
        ...
        plt.subplot(4, 5, e + 6)
        plt.imshow(
            continuity[eidx]["delay_dist"],
            origin="lower",
            extent=(0.5, 10 + 0.5, -0.5, 15 - 0.5),
            aspect="auto",
        )
        if e > 0:
            plt.xticks([])
    ...
    if savename is not None:
        saveFig(
            plt.gcf(), savename + "_OfflineTrajectoryProps", savefolder, filetype="png"
        )
...
def IsoMapFigure(
    ...
    obs, act, state, _ = predictiveNet.collectObservationSequence(
        env, agent, timesteps_wake
    )
    obs_pred, obs_next, h = predictiveNet.predict(obs, act)

    obs_pred, h_t, noise_t = predictiveNet.spontaneous(
    ...
    n_components = 3
    X = np.concatenate(
        (np.squeeze(h.detach().numpy()), np.squeeze(h_t.detach().numpy()))
    )
    ...
    ax.scatter(
        Y[: (timesteps_wake + 1), 0],
        Y[: (timesteps_wake + 1), 1],
        Y[: (timesteps_wake + 1), 2],
        c=color,
        marker=".",
        s=5,
    )
    ...
    plt.subplot(4, 4, 3)
    plt.scatter(
        state["agent_pos"][:, 0], state["agent_pos"][:, 1], c=color, marker=".", s=5
    )
    plt.xticks([])
    plt.yticks([])
