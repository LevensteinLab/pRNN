...
class OfflineTrajectoryAnalysis:
    def __init__(
        ...
        timesteps_wake=2000,
        suppressPrint=False,
        withTransitionMaps=True,
        skipContinuity=False,
        sleepOnsetTransient=0,
        compareWake=False,
        compareDiffusion=False,
        decoderbatches=10000,
        wakeAgent=None,
        withAdapt=False,
        b_adapt=0.5,
        tau_adapt=8,
        calculateDiffusionFit=True,
        calculateViewSimilarity=False,
    ):
        ...
        # this is for backward compatibility, better provide the wakeAgent as an arg
        if not wakeAgent:
            action_probability = np.array([0.15, 0.15, 0.6, 0.1, 0, 0, 0])
            wakeAgent = RandomActionAgent(env.action_space, action_probability)

        if decoder == "train":
            self.decoder = self.trainDecoder(
                decoderbatches, wakeAgent, trainHDDecoder=calculateViewSimilarity
            )
        else:
            self.decoder = decoder

        self.SLEEPactvity = self.runSLEEP(
            noisemag,
            noisestd,
            timesteps_sleep,
            actionAgent,
        ...
        self.SLEEPdecoded = self.decodeSLEEP(self.SLEEPactvity["h"], self.decoder)
        self.coverageContinuity = self.calculateCoverageContinuity(
            self.SLEEPdecoded, skipContinuity=skipContinuity
        )
        ...
        self.transitionMaps = False
        if withTransitionMaps:
            self.transitionMaps = self.calculateTransitionMaps(
                self.SLEEPdecoded, self.SLEEPactvity
            )
        if compareWake or withIsomap or calculateViewSimilarity:
            self.WAKEactity = self.runWAKE(env, wakeAgent, timesteps_wake)

        self.compareWake = compareWake
        if compareWake:
            # WAKEdecoded = self.decodeSLEEP(self.WAKEactity['h'],self.decoder)
            state_nap = state2nap(self.WAKEactity["state"])
            self.coverageContinuity_WAKE = self.calculateCoverageContinuity(
                (state_nap, None), skipContinuity=skipContinuity
            )

            self.SWsimilarity = self.compareSW(
                self.coverageContinuity[1], self.coverageContinuity_WAKE[1]
            )

        if compareDiffusion:
            continuity_DIFFUS = self.makeDiffusion()
            self.SDsimilarity = self.compareSW(
                self.coverageContinuity[1], continuity_DIFFUS
            )

        self.compareViewSimilarity = calculateViewSimilarity
        if calculateViewSimilarity:
            self.ViewSimilarity = self.calculateViewSimilarity(
                env, self.WAKEactity, self.SLEEPactvity
            )
    ...
    def calculateViewSimilarity(self, env, WAKEactivity, SLEEPactivity):
        ...
        decodedViewObs, decoded = getDecodedViewObs(self.pN, h, self.decoder)
        # error = decodedViewObs.detach().numpy() - WAKEactivity['obs_pred'].detach().numpy()
        # MSE_wake = np.mean(np.square(error),axis=2)
        (MSE_wake, mean_wake, std_wake) = delayCorr(
            np.squeeze(decodedViewObs.detach().numpy()).T,
        ...
        (MSE_real, mean_real, std_real) = delayCorr(
            np.squeeze(WAKEactivity["obs_next"].detach().numpy()).T,
            np.squeeze(WAKEactivity["obs_pred"].detach().numpy()).T,
        )
        ...
        (MSE_sleep, mean_sleep, std_sleep) = delayCorr(
            np.squeeze(decodedViewObs.detach().numpy()).T,
    ...
    def calculateDiffusionFit(decodedpos, dtmax=15):
        ...
        _, meandistsq = delaydist(
            decodedpos.values, dtmax, sqdist=True, dist="euclidian"
        )
    ...
    def calculateTransitionMaps(self, decoded, activity, transrange=3):
        # Not done for the continuous envs yet
        decodedpos = decoded[0].values
        transitionprob, edges = self.calculateTransitionProbability(
            decodedpos, transrange=transrange
        )
        ...
        if type(activity["noise_t"]) is tuple:
            ...
            for act in range(numacts):
                ...
                for HD in range(numHDs):
                    ...
                    transitionprob, edges = self.calculateTransitionProbability(
                        decodedpos, restrict=restrict, transrange=transrange
                    )
    ...
    def calculateTransitionProbability(self, decodedpos, restrict=None, transrange=3):
    ...
    def transitionProbabilityPanel(
        self, transitionprob, marker="+", vmax=None, incLabels=True
    ):
    ...
    def transitionProbabilityFigure(self, netname, savefolder):
        ...
        for act in range(numacts):
            ...
            self.transitionProbabilityPanel(
                (transitionMaps["HDaligned"][act], transitionMaps["edges"]),
                marker="^",
                vmax=0.2,
                incLabels=False,
            )
            ...
            for HD in range(numHDs):
                ...
                self.transitionProbabilityPanel(
                    (transitionMaps["AllTransProb"][act][HD], transitionMaps["edges"]),
        ...
        if netname is not None:
            saveFig(
                plt.gcf(),
                netname + "_TransitionProbability",
                savefolder,
                filetype="png",
            )
        # plt.show()

    def trainDecoder(self, numbatches=10000, trainHDDecoder=False, agent=None):
        env = self.pN.EnvLibrary[0]
        _, _, decoder = self.pN.calculateSpatialRepresentation(
            env,
            agent,
    ...
    def runSLEEP(
        ...
        a = {}
        a["obs_pred"], a["h"], a["noise_t"] = self.pN_sleep.spontaneous(
            timesteps_sleep, noisemag, noisestd, agent=actionAgent
        )
    ...
    def runWAKE(self, env, agent, timesteps_wake, theta="expand"):
        print("Running WAKE")
        a = {}
        a["obs"], a["act"], a["state"], _ = self.pN.collectObservationSequence(
            env, agent, timesteps_wake
        )
        ...
        if theta == "expand":
            ...
            a["obs_pred"] = (
                a["obs_pred"]
                .transpose(0, 1)
                .reshape((-1, 1, a["obs_pred"].size(dim=2)))
                .swapaxes(0, 1)
            )
            a["obs_next"] = (
                a["obs_next"]
                .transpose(0, 1)
                .reshape((-1, 1, a["obs_next"].size(dim=2)))
                .swapaxes(0, 1)
            )
    ...
    def calculateCoverageContinuity(self, decoded, skipContinuity=False):
        ...
        coverage = trajectoryAnalysis.calculateCoverage(
            decoded, [0, self.decoder.gridheight, 0, self.decoder.gridwidth]
        )
        ...
        if p is not None:
            ...
            pdhist, pbinedges, dtbinedges = np.histogram2d(
                dx, np.log10(maxp[:-1]), bins=[np.arange(0, 15), np.linspace(-1, 0, 11)]
            )
    ...
    def fitIsomap(self):
        ...
        # X = self.WAKEactity['h']
        h_wake = self.WAKEactity["h"]
        h_sleep = np.squeeze(self.SLEEPactvity["h"].detach().numpy())
    ...
    def SpontTrajectoryFigure(self, netname, savefolder, trajRange=(0, 20)):
        ...
        obs_pred = self.SLEEPactvity["obs_pred"]
        obs_pred = self.pN.env_shell.pred2np(
            obs_pred[:, range(trajRange[0], trajRange[0] + 10), ...]
        )
        h = self.SLEEPactvity["h"]
        decoded, _ = self.SLEEPdecoded
        if self.transitionMaps:
            transitionprob = (
                self.transitionMaps["transitionprob"],
        ...
        self.pN.plotSequence(obs_pred, range(10), 5, label="Predicted", numrows=9)

        self.pN.plotSequence(
            np.transpose(np.log10(p), axes=[0, 2, 1]),
            range(trajRange[0], trajRange[0] + 10),
            4,
            label="Decoded",
        ...
        if self.compareViewSimilarity:
            ...
            obs_view = self.pN.env_shell.pred2np(
                obs_view[:, range(trajRange[0], trajRange[0] + 10), ...]
            )
            self.pN.plotSequence(obs_view, range(10), 6, label="Predicted", numrows=9)
        # plt.colorbar()
        plt.subplot(6, 5, 5)
        ...
        if self.transitionMaps:
            plt.subplot(3, 3, 8)
            self.transitionProbabilityPanel(transitionprob, vmax=0.2)
        ...
        if netname is not None:
            saveFig(
                plt.gcf(),
                netname + "_SpontaneousTrajectory",
                savefolder,
                filetype="png",
            )
    ...
    def compareViewStatsPanel(self, ViewSimilarity):
        ...
        plt.boxplot(
            [np.squeeze(MSE_real), np.squeeze(MSE_wake), np.squeeze(MSE_sleep)],
    ...
    def diffusionPanel(self, diffusionFit):
        ...
        plt.plot(
            [0, np.log10(delays[-1])],
            [intercept, intercept + alpha * np.log10(delays[-1])],
            "r",
        )
        plt.text(0.75, intercept, f"{np.round(alpha,2)}", fontsize=10, color="r")
        plt.xlabel("log(time)")
    ...
    def isomapPanel(self, duration=20):
        ...
        state = self.WAKEactity["state"]
        c_WAKE = np.arctan(state["agent_pos"][:-1, 0] / state["agent_pos"][:-1, 1])

        X = np.squeeze(self.SLEEPactvity["h"].detach().numpy())
        Y_SLEEP = self.Isomap.transform(X)
        c_SLEEP = np.ones((X.shape[0]))

        ax = plt.gca()
        ax.scatter(Y_WAKE[:, 0], Y_WAKE[:, 1], Y_WAKE[:, 2], c=c_WAKE, marker=".", s=4)
        ax.scatter(
            Y_SLEEP[:, 0],
            Y_SLEEP[:, 1],
            Y_SLEEP[:, 2],
        ...
        ax.plot3D(
            Y_SLEEP[:duration, 0], Y_SLEEP[:duration, 1], Y_SLEEP[:duration, 2], "k"
        )
        ax.scatter(
            Y_SLEEP[:duration, 0],
            Y_SLEEP[:duration, 1],
    ...
    def trajectoryPanel(self, decoded, trajRange=(0, 20)):
        ...
        y = decoded["y"].values
        plt.imshow(
            np.ones_like(self.decoder.mask.transpose()),
            interpolation="nearest",
            alpha=self.decoder.mask.transpose(),
        )
        plt.plot(x[trajRange[0] : trajRange[1]], y[trajRange[0] : trajRange[1]], "r")
        plt.scatter(
            x[trajRange[0] : trajRange[1]],
            y[trajRange[0] : trajRange[1]],
            c=range(trajRange[1] - trajRange[0]),
            marker=".",
            cmap="cubehelix",
        )
        plt.colorbar()
    ...
    def pdPanel(self, pdhist):
        ...
        plt.imshow(
            pdhist[0:9, :], origin="lower", extent=(-1, -0, -0.5, 9.5), aspect="auto"
        )
        plt.xlabel("log p_x")
        plt.ylabel("dx")
...
def getDecodedViewObs(pN, h, decoder):
    decoded, p = pN.decode(h, decoder, withHD=True)

    decodedViewObs = []
    for decodedView in decoded.values:
        fromdecode = pN.env_shell.get_viewpoint(decodedView[0:2], decodedView[2])
        decodedViewObs.append(fromdecode.flatten())
...
def makeAdaptingNet(pN, b, tau_a):
    ...
    if isinstance(pN.pRNN.rnn.cell, LayerNormRNNCell):
        ...
        adapting_pN.pRNN.rnn.cell = AdaptingLayerNormRNNCell(
            input_size, hidden_size, musig
        )
