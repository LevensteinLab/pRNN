...
class vRNN(pRNN):
    """
    Vanilla RNN with next step prediction, no masks or LayerNorm.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=RNNCell):
...
class vRNN_0win(pRNN):
    """
    Next step prediction, no observation masking, no actions masked, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
...
class vRNN_1win(pRNN):
    """
    Next step prediction, t+1 observation masked, no actions masked, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
        super(vRNN_1win, self).__init__(obs_size, act_size, hidden_size=hidden_size,
                          cell=cell, bptttrunc=bptttrunc, neuralTimescale=neuralTimescale, dropp=dropp,
                          predOffset=1, actOffset=0,
                          inMask=[True,False], outMask=[True,True], actMask=None)

class vRNN_2win(pRNN):
    """
    Next step prediction, 2 observations masked, no actions masked, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class vRNN_3win(pRNN):
    """
    Next step prediction, 3 observations masked, no actions masked, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class vRNN_4win(pRNN):
    """
    Next step prediction, 4 observations masked, no actions masked, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class vRNN_5win(pRNN):
    """
    Next step prediction, 5 observations masked, no actions masked, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class vRNN_1win_mask(pRNN):
    """
    Next step prediction, 1 observations masked, 1 action masked, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class vRNN_2win_mask(pRNN):
    """
    Next step prediction, 2 observations masked, 2 action masked, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class vRNN_3win_mask(pRNN):
    """
    Next step prediction, 3 observations masked, 3 action masked, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class vRNN_4win_mask(pRNN):
    """
    Next step prediction, 4 observations masked, 4 actions masked, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class thRNN(pRNN):
    """
    __ RNN with no prediction offset, masked observations.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=RNNCell):
...
class thRNN_0win_noLN(pRNN):
    """
    Current-step prediction, no masks, no LayerNorm.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
...
class thRNN_0win(pRNN):
    """
    Current-step prediction, no masks, LayerNorm'd
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15,
                f=0.5):
...
class thRNN_1win(pRNN):
    """
    Current-step prediction, 1 observation masked, no actions masked, LayerNorm'd
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15,
                f=0.5, **cell_kwargs):
...
class thRNN_1win_mask(pRNN):
    """
    Current-step prediction, 1 observation masked, 1 action masked, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class thRNN_2win_mask(pRNN):
    """
    Current-step prediction, 2 observation masked, 2 action masked, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class thRNN_3win_mask(pRNN):
    """
    Current-step prediction, 3 observation masked, 3 action masked, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class thRNN_0win_prevAct(pRNN):
    """
    Current-step prediction, no observations or actions masked, actions moved back by 1, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15,
                f=0.5):
...
class thRNN_1win_prevAct(pRNN):
    """
    Current-step prediction, 1 observation masked, actions moved back 1, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15,
                f=0.5):
...
class thRNN_2win_prevAct(pRNN):
    """
    Current-step prediction, 2 observation masked, actions moved back 1, LayerNorm'd.
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15,
                f=0.5):
...
class AutoencoderRec(pRNN):
    ...
    No masks for observations or actions, no action offset. No LayerNorm
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=RNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15,
                f=None):
...
class AutoencoderPred(pRNN): 
    """
    Autoencoder Predictive, next step prediction, no obs or action masks, no offset. no LayerNorm
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=RNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15,
                f=None):
...
class AutoencoderFFPred(pRNN):
    """
    Autoencoder Feed Forward, next step prediction, no obs or action masks, no offset. no LayerNorm
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=RNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15,
                f=None):
...
class AutoencoderFF_LN(pRNN):
    """
    Autoencoder Feed Forward, current step prediction, no obs or action masks, no offset. Yes LayerNorm
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class AutoencoderRec_LN(pRNN):
    """
    Autoencoder Recurrent, current step prediction, no obs or action masks, no offset. Yes LayerNorm
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class AutoencoderFFPred_LN(pRNN):
    """
    Autoencoder Feed Forward, next step prediction, no obs or action masks, no offset. Yes LayerNorm
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
...
class AutoencoderMaskedO(pRNN):
    """
    Autoencoder recurrent, current step prediction, only next 1 observation masked, no actions masked, no predictiction at time t, no offset. Yes LayerNorm
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class AutoencoderMaskedOA(pRNN):
    """
    Autoencoder recurrent, current step prediction, next 1 observation masked, next 1 action masked, no prediction at time t, no offset. Yes LayerNorm
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class AutoencoderMaskedO_noout(pRNN):
    """
    Autoencoder recurrent, current step prediction, next 1 observation masked, no actions masked, no output preidction masked, no offset. Yes LayerNorm
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class AutoencoderMaskedOA_noout(pRNN):
    """
    Autoencoder recurrent, current step prediction, next 1 observation masked, 1 actions masked, no output preidction masked, no offset. Yes LayerNorm
    """
    def __init__(self, obs_size, act_size, hidden_size=500,
                      cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15):
...
class thcycRNN_5win_holdc(pRNN_th):
    """
    Rollout RNN; k = 5 predictions per timestep, hold action the same through rollout, continue to t+1 after kth rollout.
    """
    def __init__(self,obs_size, act_size, hidden_size=500,
                 cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15,
                f=0.5):
...
class thcycRNN_5win_firstc(pRNN_th):
    """
    Rollout RNN; k = 5 predictions per timestep, use first action then stop action input through rollout, continue to t+1 after kth rollout.
    """
    def __init__(self,obs_size, act_size, hidden_size=500,
                 cell=LayerNormRNNCell, bptttrunc=100, neuralTimescale=2, dropp = 0.15,
                f=0.5):
...
class vRNN_CANN(vRNN):
    """
    Hard coded/wired continuous attractor neural network
    """
    def __init__(self,obs_size, act_size, hidden_size=500,
                 nonlinearity='relu', bias=False):
