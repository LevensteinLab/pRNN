...
class RepresentationalConnectivityAnalysis:
    ...
    def adjustThreshold(self, newthresh, thtype="reliability"):
        ...
        if self.thtype == "reliability":
            overthresh = (self.pairData["reliability1"] > self.SIthresh) & (
                self.pairData["reliability2"] > self.SIthresh
            )
            # overthresh = self.pairData['meanReliability']>self.SIthresh

        elif self.thtype == "SI":
            overthresh = (self.pairData["SI 1"] > self.SIthresh) & (
                self.pairData["SI 2"] > self.SIthresh
            )
            # overthresh = self.pairData['meanSI']>self.SIthresh

        self.goodPairData = self.pairData[overthresh]
        self.examples = self.getExamples(self.goodPairData)
        self.weightdist = self.calculateWeightByPercentile(
            self.pairData, metric=self.thtype
        )
        self.weightdist_value = self.calculateWeightByValue(
            self.goodPairData, metric="SI"
        )
        self.weightcorr = self.calculateWeightDistRelationship(
            self.goodPairData, metric="SI", numquantiles=4
        )
    ...
    def getExamples(self, goodPairData, farThresh=8, closeThresh=3):
        ...
        farPair = goodPairData[
            (goodPairData["cellIDX2"] == excell)
            & (goodPairData["PeakDist"] > farThresh)
        ].iloc[0]
        closePair = goodPairData[
            (goodPairData["cellIDX2"] == excell)
            & (goodPairData["PeakDist"] < closeThresh)
            & (goodPairData["PeakDist"] > 0)
        ].iloc[0]
    ...
    def buildPairData(self, pN, rel_thresh=0.5):
        ...
        for tcIDX1, tc1 in enumerate(tuningCurves.values()):
            for tcIDX2, tc2 in enumerate(tuningCurves.values()):
                ...
                if (TCreliability[tcIDX1] >= rel_thresh) & (
                    TCreliability[tcIDX2] >= rel_thresh
                ):
                    (peakdist, peakloc, peakval, xcorr) = (
                        self.getRepresentationalDistance(tc1, tc2)
                    )
                else:
                    (peakdist, peakloc, peakval, xcorr) = (
                        np.nan,
                ...
                df1 = pd.DataFrame(
                    {
                        ...
                        "meanReliability": np.mean(
                            [TCreliability[tcIDX1], TCreliability[tcIDX2]]
                        ),
                        "TC 1": [tc1],
                        "TC 2": [tc2],
                        "cellIDX1": [tcIDX1],
                ...
                df2 = pd.DataFrame(
                    {
                        ...
                        "meanReliability": np.mean(
                            [TCreliability[tcIDX2], TCreliability[tcIDX1]]
                        ),
                        "TC 1": [tc2],
                        "TC 2": [tc1],
                        "cellIDX1": [tcIDX2],
                ...
                pairData = pd.concat([pairData, df1])
                pairData = pd.concat([pairData, df2])
    ...
    def getRepresentationalDistance(self, tc1, tc2):
        ...
        peakloc = np.unravel_index(np.argmax(xcorr, axis=None), xcorr.shape)
    ...
    def RepConnectFigure(self, netname=None, savefolder=None, showPercent=False):
        ...
        if netname is not None:
            saveFig(plt.gcf(), netname + "_RepConnectivity", savefolder, filetype="png")
        # plt.show()
    ...
    def TuningCurvePanel(self, tc):
        plt.imshow(
            tc.transpose(), interpolation="nearest", alpha=self.PFmask.transpose()
        )
        plt.axis("off")

    def XCorrPanel(self, singlePairData):
        plt.imshow(singlePairData["XCorr"].transpose(), extent=(-15, 15, -15, 15))
        plt.plot([0, 0], plt.ylim(), "r--", linewidth=1)
        plt.plot(singlePairData["PeakLoc"][0], -singlePairData["PeakLoc"][1], "r+")
    ...
    def AllWeightspanel(self, pairData):
        ...
        plt.scatter(
            pairData["PeakDist"] + randoffset,
            pairData["weight"],
            c=np.log10(pairData["cmap"]),
            s=1,
        )
    ...
    def calculateWeightByPercentile(
        self, pairData, metric="reliability", numquantiles=10, maxdist=11
    ):
        ...
        for p in range(numquantiles):
            quantdata = pairData[
                idxdata.between(
                    *idxdata.quantile([quantiles[p], quantiles[p + 1]]).values,
                    inclusive="both",
                )
            ]
    ...
    def calculateWeightByValue(
        self, pairData, metric="reliability", numquantiles=5, maxdist=13
    ):
        if metric == "reliability":
            idxdata = pairData["meanReliability"]
            bounds = [0, 1]
        ...
        for p in range(numquantiles):
            quantdata = pairData[
                idxdata.between(quantiles[p], quantiles[p + 1], inclusive="both")
            ]
    ...
    def WeightPercentilePanel(self, weightdist):
        plt.imshow(weightdist, vmin=-0.08, vmax=0.08, cmap="coolwarm", origin="lower")
        plt.colorbar(label="mean weight", location="right")
        plt.xlabel("Distance")
        plt.ylabel("Percentile")
        # plt.title('Next Step (LN)')

    def calculateWeightDistRelationship(
        self, pairData, numquantiles=5, metric="reliability", bounds=[0, 1]
    ):
        if metric == "reliability":
        ...
        for p1 in range(numquantiles):
            for p2 in range(numquantiles):
                inidx = (
                    idxdata1.between(quantiles[p1], quantiles[p1 + 1], inclusive="both")
                ) & (
                    idxdata2.between(quantiles[p2], quantiles[p2 + 1], inclusive="both")
                )

                weightdistcorr[p1, p2] = pairData.PeakDist[inidx].corr(
                    pairData.weight[inidx], method="spearman"
                )
