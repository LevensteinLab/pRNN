...
class representationalGeometryAnalysis:
    def __init__(
        ...
        self.cont = env.continuous
        self.max_dist = env.max_dist
        self.start_pos = (
            start_pos  # the numbering of occupiable locations starts from this
        )
        ...
        self.WAKEactivity = self.runWAKE(
            env,
            agent,
            timesteps_wake,
            theta=theta,
        ...
        if timesteps_sleep > 0:
            ...
            self.SWdist_cb = self.calculateSleepWakeDist(
                self.WAKEactivity["h"], self.SLEEPactivity["h"], metric="cityblock"
            )
            self.SWdist_cs = self.calculateSleepWakeDist(
                self.WAKEactivity["h"], self.SLEEPactivity["h"], metric="cosine"
            )

        self.Isomap = withIsomap
        if self.Isomap:
            self.Isomap = self.fitIsomap(
                self.WAKEactivity, self.SLEEPactivity, n_neighbors=n_neighbors
            )
    ...
    def calculateRSA_action(self, WAKEactivity, metric=defaultMetric):
        ...
        numacts, _ = np.histogram(
            actID, bins=np.linspace(-0.5, np.max(actID) + 0.5, np.max(actID) + 2)
        )
        ...
        (hist2, sbins, rbins) = np.histogram2d(
            dists, actdist, bins=[np.linspace(0, goodmax, 25), np.arange(-0.5, 2.5, 1)]
        )
    ...
    def actionDistancePanel(self, RSA):
        ...
        plt.imshow(ndists[np.ix_(actsort, actsort)], vmin=vmin, vmax=vmax)
    ...
    def ActionRSAFigure(self, netname, savefolder):
        ...
        saveFig(plt.gcf(), "ActionRSA_" + netname, savefolder, filetype=self.fig_type)
        # plt.show()
    ...
    def runWAKE(
        ...
        print("Running WAKE")
        a = {}
        a["obs_env"], a["act_env"], a["state"], _ = self.pN.collectObservationSequence(
            env,
            agent,
            timesteps_wake,
            obs_format=None,
    ...
    def runSLEEP(self, noisemag, noisestd, timesteps_sleep):
        print("Running SLEEP")
        a = {}
        a["obs_pred"], h_t, a["noise_t"] = self.pN.spontaneous(
            timesteps_sleep, noisemag, noisestd
        )
    ...
    def fitIsomap(self, WAKEactivity, SLEEPactivity, usecells=None, n_neighbors=150):
        ...
        method = manifold.Isomap(
            n_neighbors=n_neighbors, n_components=2, metric="cosine"
        )
        method.fit(X)
        return method

    @staticmethod
    def calculateNeuralDistWAKE(WAKEactivity_h, metric=defaultMetric, usecells=None):
    ...
    def calculateRSA_space(
        ...
        dists, keepIDX = self.calculateNeuralDistWAKE(
            WAKEactivity["h"], metric, usecells
        )
        sp_dists = self.calculateSpatialDist(
            WAKEactivity["state"], keepIDX, metric=spacemetric
        )
        ...
        if cont:
            (hist2, sbins, rbins) = np.histogram2d(
                dists,
                sp_dists,
                bins=[np.linspace(0, goodmax, 50), np.arange(0, max_dist * 0.8, 0.1)],
            )
        else:
            (hist2, sbins, rbins) = np.histogram2d(
                dists,
                sp_dists,
                bins=[np.linspace(0, goodmax, 50), np.arange(-0.5, 16.5, 1)],
            )
        hist2 = hist2 / np.sum(hist2, axis=0)
    ...
    def calculateHDDist(self, WAKEactivity_state, keepIDX=None):
        ...
        if self.cont:
            hd_dists = pdist(hd)
            hd_dists[hd_dists > np.pi] = (
                2 * np.pi - hd_dists[hd_dists > np.pi]
            )  # Circular
        else:
    ...
    def calculateRSA_obs(self, WAKEactivity, metric=defaultMetric, usecells=None):
        dists, keepIDX = self.calculateNeuralDistWAKE(
            WAKEactivity["h"], metric, usecells
        )
        ...
        for key, value in locationGroups.items():
            (hist2, sbins, rbins) = np.histogram2d(
                dists[value],
                obs_dists[value],
                bins=[np.linspace(0, goodmax, 50), np.arange(-0.01, 0.21, 0.02)],
            )
            hist2 = hist2 / np.sum(hist2, axis=0)
    ...
    def calculateRSA_HD(self, WAKEactivity, metric=defaultMetric, usecells=None):
        dists, keepIDX = self.calculateNeuralDistWAKE(
            WAKEactivity["h"], metric, usecells
        )
        ...
        hist2, sbins, rbins = np.histogram2d(
            dists, HD_dists, bins=[np.linspace(0, goodmax, 50), np.arange(-0.5, 3.5, 1)]
        )
        hist2 = hist2 / np.sum(hist2, axis=0)
    ...
    def calculateSIdependence(
        ...
        for sIDX, SIthreshold in enumerate(SIvals):
            ...
            (RSA_SIthresh[sIDX], _), _, _, _ = self.calculateRSA_space(
                WAKEactivity,
                metric=metric,
                usecells=overthresh,
                cont=cont,
        ...
        exampleSI_RSA = self.calculateRSA_space(
            self.WAKEactivity,
            metric=metric,
            usecells=excells,
            cont=cont,
    ...
    def calculateTuningCurveControl(
        self, WAKEactivity, metric=defaultMetric, cont=False, max_dist=False
    ):
        # FAKEactivity = copy.deepcopy(WAKEactivity)
        FAKEactivity = {"state": WAKEactivity["state"]}
        FAKEactivity = self.makeFAKEdata(WAKEactivity)

        RSA_fake = self.calculateRSA_space(
            FAKEactivity, metric, cont=cont, max_dist=max_dist
        )
        return RSA_fake
    ...
    def makeFAKEdata(self, WAKEactivity, useMstats=False):
        ...
        for cell, (k, tuning_curve) in enumerate(tuning_curves.items()):
            FAKEactivity["h"][:, cell] = tuning_curve[
                position[:-1, 0] - self.start_pos, position[:-1, 1] - self.start_pos
            ]

        # Calculate the tuning-curve reliability

        FAKEcorr = spearmanr(WAKEactivity["h"], FAKEactivity["h"], axis=0)
        if (
            useMstats
            and FAKEcorr.correlation.size == 1
            and np.isnan(FAKEcorr.correlation).all()
        ):
            print(FAKEcorr.correlation)
            print("correlation nan, using mstats (slower)")
            FAKEcorr = spearmanr_m(WAKEactivity["h"], FAKEactivity["h"], axis=0)
            print(FAKEcorr.correlation)
    ...
    def spatialRSApanel(self, RSA, unitlabel, colorbar=True, vmin=None, vmax=None):
    ...
    def runControlAnalyses(
        ...
        self.SIdep = self.calculateSIdependence(
            self.WAKEactivity,
            metric=metric,
            maxSI=maxSI,
            exampleSI=exampleSI,
        ...
        self.SIdep_fake = self.calculateSIdependence(
            self.FAKEactivity,
            metric=metric,
            maxSI=maxSI,
            exampleSI=exampleSI,
        ...
        self.RSA_fake = self.calculateTuningCurveControl(
            self.WAKEactivity, metric="cityblock", cont=cont, max_dist=max_dist
        )
    ...
    def SIDependenceFigure(self, netname=None, savefolder=None, excells=[0, 1]):
        ...
        if netname is not None:
            saveFig(
                plt.gcf(), "SIDependence_" + netname, savefolder, filetype=self.fig_type
            )
        # plt.show()
    ...
    def isomapPanel3d(self, colorvar="position", rotate=(0, 0)):
        ...
        if colorvar == "position":
            ...
            color = np.concatenate(
                (np.ones((X.shape[0])), np.zeros((h_sleep.shape[0])))
            )
        ...
        ax = plt.gca()
        ax.scatter(Y[:, 0], Y[:, 1], Y[:, 2], c=color, marker=".", s=4)
    ...
    def isomapPanel(self, colorvar="position", onsetTransient=5):
        ...
        if colorvar == "position":
            state = self.WAKEactivity["state"]
            color = np.arctan(
                (state["agent_pos"][:-1, 0] - self.mapcenter[0])
                / (state["agent_pos"][:-1, 1] - self.mapcenter[1])
            )
        elif colorvar == "SleepWake":
        ...
        ax = plt.gca()
        ax.scatter(Y[:, 0], Y[:, 1], c=color, marker=".", s=4)
        # ax.view_init(rotate[0]-140, rotate[1]+60)
        ax.xaxis.set_ticks([])
        ax.yaxis.set_ticks([])
        ax.axis("off")
    ...
    def keyPanel(self):
        ...
        color = np.arctan(
            (pos[:, 0] - self.mapcenter[0]) / (pos[:, 1] - self.mapcenter[1])
        )

        ax = plt.gca()
        ax.scatter(pos[:, 0], pos[:, 1], c=color, marker=".", s=4)
        # ax.view_init(rotate[0]-140, rotate[1]+60)
        ax.xaxis.set_ticks([])
        ax.yaxis.set_ticks([])
        ax.axis("off")
    ...
    def SpatialRSAFigure(self, netname=None, savefolder=None):
        ...
        if netname is not None:
            saveFig(
                plt.gcf(), "SpatialRSA_" + netname, savefolder, filetype=self.fig_type
            )
        # plt.show()
    ...
    def AllRSAFigure(self, netname, savefolder):
        ...
        plt.tight_layout()
        saveFig(plt.gcf(), "AllRSA_" + netname, savefolder, filetype=self.fig_type)
        # plt.show()
