...
class ConCoder(nn.Module):
    ...
    def predict_latents(self, latents: torch.Tensor):
        ...
        Returns preds of shape (B*N, T, D) and the corresponding ground-truth targets of shape (B*N, Z, D).
        Requires S >= k + T.
        """
        B, S, D = latents.shape
        ...
        for i in range(N):
            idx = (i+1)*k
            # the first T targets are positive targets, corresponding to the predicted window.
            targets[:, i, :T, :]  = latents[:, idx:idx+T, :]  # B, T, D
            if idx < Z//2: # if target window is too close to start, shift it right
                targets[:, i, T:T+idx, :]  = latents[:, :idx, :]  # B, Z-x, D
                targets[:, i, T+idx:, :]  = latents[:, idx+T:Z, :]  # B, x-T, D
            elif S - (idx+T//2) < Z//2: # if target window is too close to end, shift it left
                x = Z - (S - idx + T)
                targets[:, i, T:(S-idx), :]  = latents[:, idx+T:, :]  # B, Z-x-T, D
                targets[:, i, T+(S-idx):, :]  = latents[:, S - x:S, :]  # B, x, D
            else: # normal case: target window centered on predicted window
