...
def CANNmatrix(Ncells, size, selfconnect=False, peak=1, width=1, inh=0):
    ...
    weight = expKernel(distance,width)*peak - inh # The weight exponentially decays with the distance
...
def multiCANNmatrix(Ncells, size, Nmaps, selfconnect=False, peak=1, width=1, inh=0):
    ...
    for mmap in range(Nmaps):
        w,loc = CANNmatrix(Ncells, size, selfconnect=selfconnect, peak=peak, width=width, inh=inh)
        weights += w
