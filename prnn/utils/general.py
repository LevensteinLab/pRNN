...
def saveFig(fig, savename, savepath=None, filetype="png", dpi=300):
    ...
    #    filetype = [filetype]
    # for ff in filetype:
    #    fig.savefig(f'{savepath}/{savename}.{ff}',format=ff)
    fig.savefig(f"{savepath}/{savename}.{filetype}", format=filetype, dpi=dpi)
    return
...
def delaydist(
    signal, numdelays=10, maxdist=15, firstdelay=1, sqdist=False, dist="cityblock"
):
    ...
    for delay in range(firstdelay, numdelays + 1):
        ...
        if dist == "cityblock":
            distances = np.sum(
                np.abs(signal[:lastindex, :] - signal[delay:, :]), axis=1
            )
        if dist == "euclidian":
            distances = np.sqrt(
                np.sum(np.square(signal[:lastindex, :] - signal[delay:, :]), axis=1)
            )
        delay_dist[:, delay - firstdelay], _ = np.histogram(
            distances, np.arange(-0.5, maxdist + 0.5)
        )
        delay_dist[:, delay - firstdelay] = delay_dist[:, delay - firstdelay] / np.sum(
            delay_dist[:, delay - firstdelay]
        )
        if sqdist:
            delay_kl[delay - firstdelay] = np.mean(distances**2.0)
        else:
            delay_kl[delay - firstdelay] = kl_divergence(
                delay_dist[:, delay - firstdelay], nulldist
            )
...
def state2nap(state):
    ...
    state_nap = nap.TsdFrame(
        t=np.arange(np.size(data, 0)), d=data, time_units="s", columns=("x", "y")
    )
