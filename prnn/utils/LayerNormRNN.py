...
class RNNLayer(nn.Module):
    ...
    def forward(
        ...
        if internal.size(0) == 0:  # TODO: check this
            internal = torch.zeros(1, input.size(1), device=self.cell.weight_hh.device)
...
class RNNCell(nn.Module):
    def __init__(self, input_size, hidden_size, musig=None):
        ...
        self.weight_ih = Parameter(
            torch.rand(hidden_size, input_size) * 2 * rootk - rootk
        )
        self.weight_hh = Parameter(
            torch.rand(hidden_size, hidden_size) * 2 * rootk - rootk
        )
