...
class Shell:
    ...
    def collectObservationSequence(
        ...
        if dataloader:
            ...
            obs, act, state, render = agent.getObservations(
                self,
                tsteps,
            ...
            if obs_format == "pred":  # to train right away
                obs, act = self.env2pred(
                    obs, act, state=state, device=device, compute_loss=compute_loss
                )
            elif obs_format == "npgrid":  # to save as numpy array
                nps = self.env2np(
                    obs, act, state=state, device=device, save_env=save_env
                )
                obs, act = nps[0], nps[1]
                if save_env:
                    obs_env = nps[2]
...
class GymMinigridShell(Shell):
    ...
    def env2pred(self, obs, act=None, hd_from="obs", actoffset=0, **kwargs):
        if act is not None:
            ...
            act = self.encodeAction(
                act=act, obs=hd, numSuppObs=self.numHDs, numActs=self.action_space.n
            )
    ...
    def get_map_bins(self):
        minmax = (0.5, self.width - 1.5, 0.5, self.height - 1.5)
    ...
    def show_state_traj(self, start, end, state, render, **kwargs):
        ...
        plt.plot(
            (state["agent_pos"][trajectory_ts, 0] + 0.5) * 512 / 16,
            (state["agent_pos"][trajectory_ts, 1] + 0.5) * 512 / 16,
            color="r",
        )
...
class MiniworldShell(Shell):
    def __init__(self, env, act_enc, env_key, HDbins, dx=5 / 8, **kwargs):
        ...
        self.dx = dx
        self.height = int(
            (env.unwrapped.max_z - env.unwrapped.min_z - env.padding * 2) / dx
        )
        self.width = int(
            (env.unwrapped.max_x - env.unwrapped.min_x - env.padding * 2) / dx
        )
    ...
    def getActSize(self):
        action = self.encodeAction(
            act=np.ones((2, 2)), obs=np.ones(3), nbins=self.numHDs
        )
        act_size = action.size(2)
        return act_size
    ...
    def get_map_bins(self):
        minmax = (0, self.width, 0, self.height)
    ...
    def show_state_traj(self, start, end, state, render, **kwargs):
        ...
        if "pos_continuous" in state.keys():
            ...
            pos = state["agent_pos"]
        plt.scatter(
            pos[trajectory_ts, 0] * 54 / (self.env.size + self.env.padding * 2) + 4.5,
...
class MiniworldVAEShell(MiniworldShell):
    def __init__(self, env, act_enc, env_key, vae, HDbins, dx=5 / 8, **kwargs):
        super().__init__(env, act_enc, env_key, HDbins, dx, **kwargs)
        self.encoder = vae.to(
            "cpu"
        )  # default is CPU, it's moved to cuda in the training loop
    ...
    def env2pred(
        ...
        if act is not None and not from_raw:
            ...
            act = self.encodeAction(act=act, obs=hd, nbins=self.numHDs)
    ...
    def env2np(self, obs, act=None, state=None, save_env=False, device="cpu"):
        hd = state["agent_dir"]
        if act is not None:
            act = np.array(self.encodeAction(act=act, obs=hd, nbins=self.numHDs))
...
class RatInABoxShell(Shell):
    def __init__(self, env, act_enc, env_key, speed, thigmotaxis, HDbins):
        ...
        # Create the agent
        Ag = Agent(
            self.env, {"dt": 0.1, "speed_mean": speed, "thigmotaxis": thigmotaxis}
        )
    ...
    def getActSize(self):
        action = self.encodeAction(
            act=np.ones((2, 3)), meanspeed=self.ag.speed_mean, nbins=self.numHDs
        )
        act_size = action.size(2)
        return act_size
    ...
    def save_state(self, state):
        return np.array(
            [*state["agent_pos"][-1], state["agent_dir"][-1], state["mean_vel"]]
        )
    ...
    def set_agent_dir(self, direction, vel):
        self.ag.velocity = vel * np.array([np.cos(direction), np.sin(direction)])
...
class RiaBVisionShell(RatInABoxShell):
    def __init__(
        ...
        FoV_Objects = [
            RiaB_Neurons.FieldOfViewOVCs(
                self.ag, params=FoV_params | {"object_tuning_type": x}
            )
    ...
    def getObservations(
        ...
        obs = np.concatenate(
            [
                np.array(self.vision[i].history["firingrate"])[..., None]
                for i in range(len(self.vision))
            ],
            axis=-1,
        )
    ...
    def env2pred(self, obs, act=None, **kwargs):
        if act is not None:
            act = self.encodeAction(
                act=act, meanspeed=self.ag.speed_mean, nbins=self.numHDs
            )
    ...
    def to_image(self, obs):
        ...
        for obj in range(n + 1):
            ...
            facecolor_array[:, -1] = (
                0.7
                * np.maximum(  # Make the colors increasingly transparent as the next overlays the previous
                    0,
                    np.minimum(
                        1, obs[:, obj] / (self.vision[obj].max_fr * n / (n + obj))
                    ),
                )
            )
    ...
    def show_state(self, t, fig, ax, **kwargs):
        ...
        self.ag.plot_trajectory(
            t_start=start_t, t_end=end_t, fig=fig, ax=ax, color="changing"
        )
        for i in range(len(self.vision)):
    ...
    def env2pred(self, obs, act=None, **kwargs):
        ...
        if "LRoom" in self.name:
            ...
            for i in range(1, obs.shape[-1]):
                remix += (
                    np.moveaxis(
                        np.tile(obs[..., i], [3] + [1] * (len(obs[..., i].shape))),
                        0,
                        -1,
                    )
                    * self.obs_colors[i][:3]
...
class RiaBGridShell(RatInABoxShell):
    def __init__(self, env, act_enc, env_key, speed, thigmotaxis, HDbins, Grid_params):
        super().__init__(env, act_enc, env_key, speed, thigmotaxis, HDbins)

        # Create grid cells
        np.random.seed(
            42
        )  # Otherwise there will be a discrepancy with the data from dataloader
...
class RiaBColorsGridShell(RiaBVisionShell):
    def __init__(
        self, env, act_enc, env_key, speed, thigmotaxis, HDbins, FoV_params, Grid_params
    ):
        super().__init__(env, act_enc, env_key, speed, thigmotaxis, HDbins, FoV_params)
        self.n_obs = 2
        # Create grid cells
        ...
        if "LRoom" in self.name:
            ...
            for i in range(1, obs_vis.shape[-1]):
                remix += (
                    np.moveaxis(
                        np.tile(
                            obs_vis[..., i], [3] + [1] * (len(obs_vis[..., i].shape))
                        ),
                        0,
                        -1,
                    )
