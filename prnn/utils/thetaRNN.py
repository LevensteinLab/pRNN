...
class thetaRNNLayer(nn.Module):
    ...
    def forward(
        ...
        assert not (
            input.size(0) == 0 and internal.size(0) == 0
        ...
        if state.size(0) == 0:
            state = torch.zeros(
                1, 1, self.cell.hidden_size, device=self.cell.weight_hh.device
            )
        ...
        for i in range(len(inputs)):
            ...
            for th in range(theta):  # Theta-cycle inside the sequence (1 timestep)
                if batched:
                    ...
                    out_th = out_th.permute(
                        *[i for i in range(1, len(out_th.size()))], 0
                    )
                else:
                    out_th, state_th = self.cell(
                        torch.unsqueeze(inputs[i][th + 1, :], 0),
                        internals[i][th + 1, :],
                        state_th,
                    )
                out += [out_th]
...
class RNNCell(nn.Module):
    def __init__(self, input_size, hidden_size, musig=None, **kwargs):
        ...
        self.weight_hh = Parameter(
            torch.rand(hidden_size, hidden_size) * 2 * rootk_h - rootk_h
        )
...
class AdaptingRNNCell(nn.Module):
    def __init__(self, input_size, hidden_size, musig=None, **kwargs):
        ...
        self.tau_a = 8.0  # Parameter(torch.ones(1)*8)
...
class LogNRNNCell(nn.Module):
    def __init__(
        self, input_size, hidden_size, musig=[0, 1], mean_std_ratio=1.0, sparsity=1.0
    ):
        ...
        sparse_lognormal_(self.weight_ih, mean_std_ratio=mean_std_ratio, sparsity=1.0)
...
class SparseGatedRNNCell(nn.Module):
    def __init__(
        ...
        rootk_h = np.sqrt(1.0 / hidden_size)
        rootk_i = np.sqrt(1.0 / input_size)
        rootk_s = np.sqrt(1.0 / sparse_size)
        self.weight_ih = Parameter(
            torch.rand(hidden_size, input_size) * 2 * rootk_i - rootk_i
        )
        self.weight_is = Parameter(
            torch.rand(sparse_size, input_size) * 2 * rootk_i - rootk_i
        )
        self.weight_sh = Parameter(
            torch.rand(hidden_size, sparse_size) * 2 * rootk_s - rootk_s
        )
        self.weight_hh = Parameter(
            torch.rand(hidden_size, hidden_size) * 2 * rootk_h - rootk_h
        )
        self.weight_hs = Parameter(
            torch.rand(sparse_size, hidden_size) * 2 * rootk_h - rootk_h
        )
...
def test_script_thrnn_layer(seq_len, input_size, hidden_size, trunc, theta):
    ...
    rnn_ctl = nn.RNN(
        input_size, hidden_size, batch_first=True, bias=False, nonlinearity="relu"
    )
