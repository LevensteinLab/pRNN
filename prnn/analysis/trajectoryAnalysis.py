...
def calculateCoverage(trajectory, envbounds, showFig=False, mask=None):
    ...
    binedges = [
        np.arange(envbounds[0], envbounds[1] + 1) - 0.5,
        np.arange(envbounds[2], envbounds[3] + 1) - 0.5,
    ]
    bincenters = [
        np.arange(envbounds[0], envbounds[1]),
        np.arange(envbounds[2], envbounds[3]),
    ]

    occupancy, _, _ = np.histogram2d(
        trajectory["x"].values, trajectory["y"].values, bins=binedges
    )
...
def calculateContinuity(trajectory, showFig=False, numdelays=10, maxdist=15):
    ...
    delay_dist, delay_kl = delaydist(
        trajectory.values, numdelays=numdelays, maxdist=maxdist, firstdelay=1
    )
    # A, K = fit_exp_linear(np.arange(1,numdelays+1), delay_kl, C=0)

    # For threshold - shuffle trajectories
    numshuff = 100
    _, shuff_kl = delaydist(
        np.random.permutation(trajectory.values),
        numdelays=numshuff,
        maxdist=maxdist,
        firstdelay=1,
    )
