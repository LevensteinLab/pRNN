...
from prnn.analysis.representationalGeometryAnalysis import (
    representationalGeometryAnalysis as rg,
)
...
class SpontaneousActivityAnalysis:
    def __init__(
        ...
        self.spontStats = self.runSpontaneousSims(
            self.noisemags,
            self.noisestds,
            decoder,
            timesteps,
        ...
        self.exampleSims = self.runSamples(
            examples, timesteps, decoder, self.wakeStats[2], self.wakeStats[6]
        )
    ...
    def runSim_getResults(
        ...
        _, h, _ = self.pN.spontaneous(
            timesteps, noisemag, noisestd, wgain=wgain, agent=actionAgent
        )
        ...
        if decoder:
            ...
            decoded_slice = decoded.__getitem__(
                range(onset, decoded.shape[0]), columns=decoded.columns
            )
            c_cov = trajectoryAnalysis.calculateCoverage(
                decoded_slice,
                [0, decoder.gridheight, 0, decoder.gridwidth],
                mask=decoder.mask,
            )
            coverage = 1 - c_cov["nonuniformity"]
            c_cont = trajectoryAnalysis.calculateContinuity(
                decoded_slice, showFig=False, numdelays=25, maxdist=20
            )
            continuity = c_cont["underthresh"]

            transitionmap = self.calculateTransitionMap(decoded_slice, transrange=3)
            ...
            diffusioncoeff = diffFit["alpha"]
            diffusionRsq = diffFit["r_sq"]
            diffusionG = diffFit["intercept"]
    ...
    def runSpontaneousSims(
        self, noisemags, noisestds, decoder, timesteps, h_wake, wake_continuity=None
    ):
        (
            meanrate,
            stdrate,
            ...
            diffRsq,
            diffG,
        ) = self.initalizeOutputs(noisemags, noisestds)
    ...
    def runWakeComparison(self, timesteps, decoder):
        ...
        obs_pred, obs_next, h_wake = self.pN.predict(obs, act)
        ...
        c_cov = trajectoryAnalysis.calculateCoverage(
            state_nap, [0, decoder.gridheight, 0, decoder.gridwidth], mask=decoder.mask
        )
        coverage_wake = 1 - c_cov["nonuniformity"]
        c_cont = trajectoryAnalysis.calculateContinuity(
            state_nap, showFig=False, numdelays=25, maxdist=20
        )
    ...
    def OfflineActivityFigure(self, netname, savefolder):
        ...
        if netname is not None:
            saveFig(plt.gcf(), netname + "_OfflineActivity", savefolder, filetype="png")
    ...
    def OfflineActivityStatsFigure(self, netname, savefolder):
        ...
        meanWAKE, stdWAKE, h_wake, coverage_wake, continuity_wake, c_cov, c_cont = (
            self.wakeStats
        )
        ...
        plt.subplot(4, 2, 7)
        plt.scatter(coverage, SDsim, c=SleepSimilarity, vmin=0, vmax=1)
        ...
        for s, sample in enumerate(self.exampleSims):
            plt.subplot(
                4, len(self.exampleSims) + 1, 2 + s + 2 * (len(self.exampleSims) + 1)
            )
            self.continuityPanel(sample["c_cont"], SWsim=sample["SDsim"])

            plt.subplot(
                4, len(self.exampleSims) + 1, 2 + s + 1 * (len(self.exampleSims) + 1)
            )
        ...
        plt.tight_layout()
        if netname is not None:
            saveFig(plt.gcf(), netname + "_OfflineStats", savefolder, filetype="png")
    ...
    def calculateTransitionMap(self, decoded, transrange=3):
        ...
        transitionprob, edges = (
            OfflineTrajectoryAnalysis.calculateTransitionProbability(
                self, decodedpos, transrange=transrange
            )
        )
    ...
    def transitionProbabilityPanel(self, transitionprob):
        OfflineTrajectoryAnalysis.transitionProbabilityPanel(
    ...
    def musigPanel(self, metric, WAKEcompare, logScale=False, clim=None, cmap=None):
        ...
        if WAKEcompare is not None:
            im.set_clim(
                vmin=np.log10(WAKEcompare) - 2.5, vmax=np.log10(WAKEcompare) + 2.5
            )
