...
class DiffusionReplayAnalysis:
    def __init__(
        ...
        if wakeAgent:
            self.wakeAgent = wakeAgent
        else:
            action_probability = np.array([0.15, 0.15, 0.6, 0.1, 0, 0, 0])
            self.wakeAgent = RandomActionAgent(
                self.pN.env_shell.action_space, action_probability
            )
        ...
        if compareWake:
            (self.msd_WAKE, _, self.delay_dist_WAKE, self.coverage_WAKE, _, _) = (
                self.runOfflineTrials(
                    num_trials,
                    noisemag,
                    noisestd,
            ...
            self.diffusionFit_WAKE = self.calculateDiffusionFit(
                self.msd_WAKE, self.delays
            )
    ...
    def runSTDPanel(self, minStd=0.01, maxStd=1, numStds=11):
        ...
        for nidx, noise in enumerate(noisestds):
            print(nidx)
            (msd, delays, delay_dist, coverage, coherence, extent) = (
                self.runOfflineTrials(
                    self.num_trials,
                    self.noisemag,
                    noise,
    ...
    def runOfflineTrials(
        ...
        for trial in range(num_trials):
            if wakeControl:
                ...
                spatialCoherence = {"cohere": None, "extent": None}
            else:
                actvity = self.runSLEEP(
                    noisemag,
                    noisestd,
                    timesteps_sleep,
                    actionAgent,
            ...
            delay_dist, msd[trial, :] = delaydist(
                decoded.values, dtmax, sqdist=True, dist="euclidian"
            )
    ...
    def trainDecoder(self, numbatches=10000):
        ...
        _, _, decoder = self.pN.calculateSpatialRepresentation(
            env, self.wakeAgent, numBatches=numbatches, trainDecoder=True
        )
        return decoder
    ...
    def runWAKE(self, timesteps_wake):
        ...
        a["obs"], a["act"], a["state"], _ = self.pN.collectObservationSequence(
            env, self.wakeAgent, timesteps_wake
        )
    ...
    def runSLEEP(
        ...
        # Not adapted for RiaB and farama-minigrid yet
        ...
        if actionAgent == "RandHDForward":
            env = self.pN.EnvLibrary[0]
            action_probability = np.array([0.15, 0.15, 0.6, 0.1, 0, 0, 0])
            actionAgent = RandomHDAgent(
                env.action_space, action_probability, constantAction=2
            )
    ...
    def DiffusionFigure(self, netname=None, savefolder=None, halflims=True):
        ...
        if netname is not None:
            saveFig(plt.gcf(), netname + "_DiffusionFigure", savefolder, filetype="png")

        # plt.show()

    def diffusionFitPanel(
        self,
        delays,
        msd,
        diffusionFit=None,
        ...
        if msd_wake is not None:
            ...
            if errorshade:
                plt.fill_between(
                    np.log10(delays),
                    mean_msd_W - std_msd_W,
                    mean_msd_W + std_msd_W,
                    color="grey",
                    alpha=0.1,
                )
                plt.plot(np.log10(delays), mean_msd_W, "-", color="grey")
            else:
                plt.errorbar(
                    np.log10(delays),
                    mean_msd_W,
    ...
    def STDPanelFigure(self, netname=None, savefolder=None):
        ...
        plt.plot(
            np.log10(self.STDPanel["noisestds"]),
            self.diffusionFit_WAKE["alpha"] * np.ones(numnoise),
            "k",
        )
        plt.scatter(
            np.log10(self.STDPanel["noisestds"]),
        ...
        plt.subplot(3, 3, 9)
        plt.plot(
            np.log10(self.STDPanel["noisestds"]), self.STDPanel["intercept"], "grey"
        )
        plt.plot(
            np.log10(self.STDPanel["noisestds"]),
            self.diffusionFit_WAKE["intercept"] * np.ones(numnoise),
            "k",
        )
        plt.scatter(
            np.log10(self.STDPanel["noisestds"]),
            self.STDPanel["intercept"],
            c=np.arange(numnoise),
        )
        ...
        # plt.tight_layout()
        if netname is not None:
            saveFig(plt.gcf(), netname + "_STDPanel", savefolder, filetype="png")
        # plt.show()
