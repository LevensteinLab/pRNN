...
class PredictiveNet:
    ...
    def __init__(
        ...
        **architecture_kwargs,
        ...
        self.encoder_grad = (
            encoder_grad  # wether to pass the pRNN gradients to the encoder
        )
    ...
    def reset_state(self, randInit=True, device="cpu"):
        ...
        if randInit:
            ...
            self.state = noise[0] + noise[1] * torch.randn(
    ...
    def spontaneous(
        self, timesteps, noisemean, noisestd, wgain=1, agent=None, randInit=True
    ):
    ...
    def trainStep(
        ...
        homeoloss, sparsity, meanrate = self.homeostaticLoss(
            h, target_sparsity, target_rate, decor
        )
    ...
    def homeostaticStep(
        self, h, target_sparsity=None, target_rate=None, decorrelate=False
    ):
        """
    ...
    def recordTrainingTrial(self, loss, enc_loss=None):
        ...
        try:
            self.TrainingSaver = pd.concat(
                (self.TrainingSaver, newTrial), ignore_index=True
            )
        except:
            self.TrainingSaver = pd.concat(
    ...
    def addTrainingData(self, key, data):
        if isinstance(data, pd.Series) or isinstance(data, pd.DataFrame):
            data = data.values
        if (key not in self.TrainingSaver) and (
    ...
    def resetOptimizer(
        ...
        if trainBias:
            ...
            biasparmgroup = {
                ...
                "name": "biases",
                "lr": learningRate
                * bias_lr,  # Note: most papers use same as recurrent weights...,
    ...
    def calculateSpatialRepresentation(
        ...
        if hasattr(self, "current_state"):  # easy way to check if it's CANN
            obs_pred, obs_next, h = self.predict(
                obs, act, state, fullRNNstate=fullRNNstate
            )
        else:
        ...
        if inputControl:
            if self.env_shell.n_obs == 1:
                ...
                d = np.concatenate(
                    [o.squeeze().detach().numpy()[onsetTransient:-1, :] for o in obs],
                    axis=-1,
                )
            rates_input = nap.TsdFrame(
                t=np.arange(onsetTransient, timesteps), d=d, time_units="s"
            )
        ...
        if trainDecoder:
            ...
            if trainHDDecoder:
                ...
                HDdecoder.train(
                    h_decoder, pos_decoder, batchSize=0.5, numBatches=numBatches
                )
    ...
    def decode(self, h, decoder, withHD=False):
        ...
        decoded = nap.TsdFrame(
            t=np.arange(timesteps), d=decoded, time_units="s", columns=dims
        )
    ...
    def plotSampleTrajectory(
        ...
        if hasattr(self.pRNN, "k"):
            k = self.pRNN.k
        state["agent_pos"] = state["agent_pos"][
            : state["agent_pos"].shape[0] - k + 1, :
        ]
        # h = h[0:1,:,:]
        h = torch.mean(
            h, dim=0, keepdims=True
        )  # this is what's used to train the decoder...
    ...
    def plotSpontaneousTrajectory(
        ...
        if decoder:
            ...
            lochist, _, _ = np.histogram2d(
                ...
                decoded["y"].values,
                bins=[
                    np.arange(0, decoder.gridheight) - 0.5,
                    np.arange(0, decoder.gridheight - 1),
                ],
            )
    ...
    def plotObservationSequence(
        ...
        for tt in range(numtimesteps):
            if render is not None:
                ...
                self.env_shell.show_state(
                    render=render, t=timesteps[tt], fig=fig, ax=ax
                )
            if tt == 0:
                plt.ylabel("State")
            plt.xticks([])
            ...
            if obs_pred is not None:
                plt.subplot(
                    6, numtimesteps, 2 * numtimesteps + tt + 1 + 2 * numtimesteps
                )
            ...
            if obs_next is not None:
                plt.subplot(
                    6, numtimesteps, 0 * numtimesteps + tt + 1 + 2 * numtimesteps
                )
            ...
            if p_decode is not None:
                plt.subplot(
                    6, numtimesteps, 3 * numtimesteps + tt + 1 + 2 * numtimesteps
                )
                plt.imshow(
                ...
                if state is not None:
                    plt.plot(
                        ...
                        marker=(
                            3,
                            0,
                            self.env_shell.dir2deg(state["agent_dir"][timesteps[tt]]),
                        ),
                        color="r",
                        markersize=6,
                    )
    ...
    def plotLearningCurve(
        ...
        if savename is not None:
            saveFig(
                fig, savename + "_LearningCurve", savefolder, filetype=self.fig_type
            )
        # if axis is None:
        # plt.show()
    ...
    def plotActivationTimeseries(self, h):
        ...
        plt.plot(
            actStats["poprate_t"][timeIDX] + actStats["popstd_t"][timeIDX],
            "--k",
            linewidth=1,
        )
        plt.xlabel("t")
