...
class TrajDataset(Dataset):
    def __init__(
        self, folder: str, seq_length: int, n_trajs: int, act_datatype=None, n_obs=1
    ):
...
class TrajRawDataset(TrajDataset):
    ...
    def __getitem__(self, index):
        ...
        act = torch.tensor(act, dtype=self.act_type)
        raw = np.load(self._data_dir + "/" + str(index + 1) + "/raw.npy")[
            :, : self.seq_length + 1
        ]
...
def generate_trajectories(env, agent, n_trajs, seq_length, folder, save_raw=False):
    ...
    if n_generated < n_trajs:
        ...
        for i in range(n_trajs - n_generated):
            ...
            traj_dir.mkdir()
            data = env.collectObservationSequence(
                agent, seq_length, obs_format="npgrid", save_env=save_raw, device=device
            )
...
def create_dataloader(
    ...
    if generate:
        generate_trajectories(
            env, agent, n_trajs, seq_length, folder, save_raw=save_raw
        )
    if not tmp_folder:
        tmp_folder = folder
    elif not load_raw:
        ...
        copytree(
            ...
            tmp_folder,
            dirs_exist_ok=True,
            ignore=ignore_patterns("raw.npy", "state.npy"),
        )
    else:
        tmp_folder = tmp_folder + "/" + env.name + "-" + agent.name
        copytree(
            ...
            tmp_folder,
            dirs_exist_ok=True,
            ignore=ignore_patterns("obs*", "state.npy"),
        )
    if not load_raw:
        dataset = TrajDataset(
            tmp_folder, seq_length, n_trajs, env.getActType(), env.n_obs
        )
    else:  # will need for simultaneous training of the encoder
        dataset = TrajRawDataset(
            tmp_folder, seq_length, n_trajs, env.getActType(), env.n_obs
        )
    env.addDataLoader(
        DataLoader(
    ...
    print(
        f"Dataloader created with {n_trajs} trajectories, sequence length {seq_length}"
    )
...
class MergedTrajDataset(TrajDataset):
    ...
    def __getitem__(self, index):
        ...
        for i, n in enumerate(self.n_trajs):
            try:
                act = np.load(self._data_dir[i] + "/" + str(index + 1) + "/act.npy")[
                    :, : self.seq_length
                ]
                obs = np.load(self._data_dir[i] + "/" + str(index + 1) + "/obs.npy")[
                    :, : self.seq_length + 1
                ]
                act = torch.tensor(act, dtype=self.act_type)
                obs = torch.tensor(obs, dtype=torch.float32)
...
def mergeDatasets(envs, batch_size=1, shuffle=True, num_workers=0, mixed_batch=True):
    ...
    datasetMerged = MergedTrajDataset(
        datafolders, min(seq_length), n_trajs, env_act_datatype
    )
    ...
    if not mixed_batch:
        ...
        envMerged.addDataLoader(
            DataLoader(
                datasetMerged, batch_sampler=batch_sampler, num_workers=num_workers
            )
        )
    else:
        envMerged.addDataLoader(
