...
class RandomActionAgent:
    def __init__(self, action_space, default_action_probability=None):
        ...
        if default_action_probability is None:
            self.default_action_probability = (
                np.ones_like(self.action_space) / self.action_space.n
            )
    ...
    def generateActionSequence(self, tsteps, action_probability=None):
        ...
        action_sequence = randActionSequence(
            tsteps, self.action_space, action_probability
        )
        return action_sequence
...
class RandomHDAgent:
    def __init__(
        self, action_space, default_action_probability=None, constantAction=-1
    ):
        self.action_space = action_space
        self.default_action_probability = default_action_probability
        if default_action_probability is None:
...
class RatInABoxAgent:
    ...
    def getObservations(
        ...
        obs, act, state, render = shell.getObservations(
            tsteps, reset, includeRender, discretize, inv_x, inv_y
        )
...
class MiniworldRandomAgent(Agent):
    ...
    def update(self, dt=None, drift_velocity=None, drift_to_random_strength_ratio=1):
        ...
        self.history["speed"].append(
            np.linalg.norm(
                np.array(self.history["pos"][-1]) - np.array(self.history["pos"][-2])
            )
        )
    ...
    def getObservations(
        ...
        **kwargs,
        ...
        if act is None:
            ...
            pos = (
                np.array(
                    [pos[0] - env.env.padding, env.env.size - pos[2] + env.env.padding]
                )
                / env.env.size
            )
