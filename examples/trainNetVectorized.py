...
parser.add_argument("--use_FF", default=False, type=bool, help="Make network Feed Forward only?")

parser.add_argument(
    "--mask_actions", default=False, type=bool, help="Mask actions from model input as well?"
)

parser.add_argument(
    "--actOffset", default=0, type=int, help="Number of timesteps to offset actions by (backwards)"
)
...
parser.add_argument("--rollout_action", default="full", type=str, help="Action structure")
...
if args.contin: #continue previous training, so load net from folder
    ...
    if args.env == '':
        envs = predictiveNet.loadEnvironment(args.load_env)
        predictiveNet.addEnvironment(envs)
    else:
        envs = make_env(args.env, args.envPackage, args.actenc)
        predictiveNet.addEnvironment(envs)
    ...
    predictiveNet.plotSampleTrajectory(envs,agent,
