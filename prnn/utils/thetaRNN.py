...
def xavier_init(input_size: int, hidden_size: int, W_ih: Tensor, W_hh: Tensor):
    ...
    W_ih = Parameter(
        torch.rand(hidden_size, input_size) * 2 * rootk_i - rootk_i
    )  # scales uniform dist : W ~ U(- sqrt(1/n_size), + sqrt(1/n_size)
...
class BaseCell(nn.Module, ABC):
    ...
    def __init__(
        self, input_size: int, hidden_size: int, actfun: str = "relu", *args, **kwargs
    ) -> None:
        super().__init__()
    ...
    @abstractmethod
    def forward(
        self, input: Tensor, internal: Tensor, state: Tensor, *args, **kwargs
    ) -> Tuple[Tensor, Tensor]:  # tuple of hy
...
class RNNCell(BaseCell):
    ...
    def forward(
        self, input: Tensor, internal: Tensor, state: Tensor
    ) -> Tuple[Tensor, Tensor]:  # tuple of hy
        """
        ...
        hy = self.actfun(
            x + internal + self.bias
        )  # canonical RNN hidden state update (hx --> hy) but with an *internal* term
        return hy, (hy,)
...
class AdaptingRNNCell(RNNCell):
    ...
    def forward(self, input: Tensor, internal: Tensor, state: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        ay = (
            ax * (1 - 1 / self.tau_a) + (self.b / self.tau_a) * hx
        )  # adaptation variable, a_{t} dependent on both h_{t-1} and a_{t-1}
...
class thetaRNNLayer(nn.Module):
    ...
    def __init__(
        self, cell, trunc, *cell_args, defaultTheta=0, continuousTheta=False, **cell_kwargs
    ):  # the init scheme (xavier vs lognormal) will get propagated through here
    ...
    def preprocess_inputs(self, input, internal, state, theta, batched: bool):
        ...
        assert not (input.size(0) == 0 and internal.size(0) == 0), (
            "RNN should be driven by input and/or noise."
        )
        ...
        if state.size(0) == 0:
            state = torch.zeros(1, 1, self.cell.hidden_size, device=self.cell.weight_hh.device)
        if internal.size(0) == 0:
            internal = torch.zeros(
                theta + 1, input.size(1), self.cell.hidden_size, device=self.cell.weight_hh.device
            )
        ...
        if input.size(0) < theta + 1:  # For "first-only" action inputs
            if batched:
                ...
                pad = (0, 0, 0, 0, 0, theta)
            input = nn.functional.pad(input=input, pad=pad, mode="constant", value=0)
    ...
    def forward(
        ...
        for i in range(len(inputs)):  # loop over all inputs in the sequence
            ...
            if batched:
                out, state = self.cell(
                    inputs[i][0, :].permute(1, 0), internals[i][0, :].permute(1, 0), state
                )
                out = out.unsqueeze(1)
                out = out.permute(*[i for i in range(1, len(out.size()))], 0)
            else:
                out, state = self.cell(
                    torch.unsqueeze(inputs[i][0, :], 0), internals[i][0, :], state
                )
            ...
            for th in range(
                theta
            ):  # Theta-cycle inside the sequence (1 timestep) #loop over all theta steps within the time step
                if batched:
                    out_th, state_th = self.cell(
                        inputs[i][th + 1, :].permute(1, 0),
                    ...
                    out_th, state_th = self.cell(
                        torch.unsqueeze(inputs[i][th + 1, :], 0), internals[i][th + 1, :], state_th
                    )
                out += [out_th]
            out = torch.cat(out, 0)
